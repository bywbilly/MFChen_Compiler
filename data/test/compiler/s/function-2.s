...
____getSum____function:
	sub $sp, $sp, 144
	sw $t1, 36($sp)
	sw $t0, 32($sp)

____getSum_0____enter:

____getSum_1____entry:
	lw $v1, 140($sp)
	lw $t1, 0($v1)
	lw $v1, 140($sp)
	lw $t0, 4($v1)
...
____main_1____entry:
	...
	li $v1, 7
	sw $v1, -4($sp)
	jal ____Pair____function
	sw $t0, -4($sp)
	jal ____getSum____function
	lw $t0, 8($t0)
	move $a0, $t0
